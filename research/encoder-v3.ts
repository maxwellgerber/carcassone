...
//
// v3 (experiment): v2 plus the hand heuristic's own per-kind expected values for
// each seat (city / road / monastery / farm EV, reserve bonus, total), so the net
// learns a correction to the heuristic instead of re-deriving it from the pool.
// Same constants as npc.ts evaluatePositions at the time of writing (farm 0.4,
// reserve 7 x 0.6^i, horizon 12).
...
function chance(openEdges: number, tilesLeft: number): number {
  if (openEdges <= 0) return 1;
  return Math.max(0.08, 0.92 - 0.16 * (openEdges - 1)) * Math.min(1, tilesLeft / (openEdges * 5));
}
/** Per-seat hand EV split by kind: [city, road, monastery, farm, reserve]. */
function handComponents(state: GameState, f: Features): number[][] {
  const n = state.players.length;
  const out = state.players.map(() => [0, 0, 0, 0, 0]);
  ...
  for (const cf of f.cityFeatures) {
    ...
    const p = cf.complete ? 1 : chance(cf.openEdges, tilesLeft);
    const ev = cf.complete ? full : p * full + (1 - p) * partial;
    for (const [pi, w] of share(onIt.map((m) => m.playerIdx))) out[pi]![0]! += ev * w;
  }
  for (const rf of f.roadFeatures) {
    ...
    const p = rf.complete ? 1 : chance(rf.openEdges, tilesLeft);
    const ev = p * full + (1 - p) * rf.tileCount;
    for (const [pi, w] of share(onIt.map((m) => m.playerIdx))) out[pi]![1]! += ev * w;
  }
  if (state.config.monasteryScoring) {
    for (const mf of f.monasteryFeatures) {
      ...
      for (const [pi, w] of share(onIt.map((m) => m.playerIdx))) out[pi]![2]! += ev * w;
    }
  }
  if (state.config.farmScoring) {
    const cityById = new Map(f.cityFeatures.map((c) => [c.id, c]));
    for (const ff of f.fieldFeatures) {
      ...
      if (!onIt.length) continue;
      let ev = 0;
      for (const cid of ff.cityIds) { const c = cityById.get(cid); if (c) ev += 3 * (c.complete ? 1 : chance(c.openEdges, tilesLeft) * 0.4); }
      for (const [pi, w] of share(onIt.map((m) => m.playerIdx))) out[pi]![3]! += ev * w;
    }
  }
  for (let pi = 0; pi < n; pi++) {
    const horizon = Math.min(1, tilesLeft / 12);
    let bonus = 0;
    for (let i = 0; i < state.players[pi]!.meeples; i++) bonus += 7 * Math.pow(0.6, i);
    out[pi]![4] = bonus * horizon;
  }
  return out;
}

/** Aggregate what a single seat holds on the board. */
function groupVector(state: GameState, f: Features, geo: Geo, seat: number, claimedBy: Map<string, Set<number>>, hand: number[]): number[] {
  ...
  // v3: the hand heuristic's own view of this seat
  v[42] = hand[0]! / 30; v[43] = hand[1]! / 15; v[44] = hand[2]! / 9; v[45] = hand[3]! / 20; v[46] = hand[4]! / 15;
  v[47] = (p.score + hand[0]! + hand[1]! + hand[2]! + hand[3]! + hand[4]!) / 60;
  return v;
}
...
export function encode(state: GameState, me: number, features = deriveFeatures(state)): Float32Array {
  ...
  const hand = handComponents(state, features);
  const mine = groupVector(state, features, geo, me, claimedBy, hand[me]!);
  const others = state.players.map((_, i) => i).filter((i) => i !== me).map((i) => groupVector(state, features, geo, i, claimedBy, hand[i]!));
