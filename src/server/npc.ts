...
} = { reserveValue: 7, reserveDecay: 0.6, evaluator: 'blend', blendNet: 0.45, candBlendNet: null, oppBestWeight: 0.4, chanceSlope: 0.16, chanceTilesPerEdge: 5, farmOpenFactor: 0.4, cityGrowth: 0.6, roadGrowth: 0.5, reserveHorizon: 12 };
 ...
 *  self-play outcomes (scripts/selfplay.ts + scripts/train.ts), rescaled to points. */
let net: Net | null = null;
export function netValue(state: GameState, me: number, features = deriveFeatures(state)): number {
  net ??= Net.fromJSON(WEIGHTS);
  return net.predict(encode(state, me, features)) * 40;
}

function handValue(state: GameState, me: number, features: Features): number {
